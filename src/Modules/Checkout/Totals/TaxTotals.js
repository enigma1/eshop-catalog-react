...
const TaxOptions = ({group, options, total, setPayment}) => options
...
const TaxTotals = ({modData, setPayment, group, service}) => {
  ...
  return (<>{state.ready &&
    <Fieldset className="p-3 border">
      <Legend>{title}</Legend>
      <TaxOptions setPayment={setPaymentOption} total={total} options={costs} group={group}></TaxOptions>
    </Fieldset>
  }</>)
}

TaxTotals.defaultProps = {
  name: Personal.name,
  group: 'payment',
  service: 'checkout'
}
